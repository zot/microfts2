...
- **R646:** When a transform is registered for a strategy, it is applied while indexing to every chunk that strategy produces from raw file bytes; it shapes only the trigram/token index and the derived `Attrs`, never what retrieval returns
- **R647:** On index paths (add, reindex, append) the transform's stripped `Content` feeds trigram extraction and tokenization and its derived `Attrs` are recorded for the chunk, while the C record stores the original chunker `Content` (its `ContentLen` and dedup hash reflect the original)
...
- **R649:** The transform applies to overlay (`tmp://`) chunks on the index paths (add, update, append) only; overlay retrieval re-chunks the stored raw bytes and returns the original content without the transform
- **R650:** The chunk's `Range` spans the raw file region, including any bytes the transform strips on the index path, so retrieval re-reads that region and returns the original content
...
- **R655:** Retrieval paths — `GetChunks` (streaming fallback and `RandomAccessChunker` fast path), `ChunkCache`, and `tmp://` — return the original chunker `Content`; the transform is not applied on retrieval, and retrieval `Attrs` are read from the stored C record
- **R656:** The chunk dedup hash is SHA-256 over the chunk's original `Content` alone; derived `Attrs` do not affect the hash
- **R657:** Chunk dedup identity is the original `Content`: identical original `Content` dedups to one chunkid regardless of derived `Attrs`, and differing original `Content` (including a differing tag line) yields distinct chunkids
- **R658:** On append, a chunk whose original `Content` changed (e.g. a tag line added or edited) hashes differently, producing a fresh chunkid and firing `WithIndexedChunkCallback` — a tag edit re-indexes naturally because the tag text is part of the original `Content`
- **R659:** `WithIndexedChunkCallback` delivers the original chunker `Content` together with the derived `Attrs` (matching the C record), consistent with the retrieval paths
