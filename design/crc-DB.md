...
- transforms: map[string]ContentTransform — per-strategy content transforms registered alongside chunkers via AddChunker; absent/nil = no transform. R643, R644
...
- RemoveCallback: exported type `func(txn *lmdb.Txn, orphanedChunkIDs []uint64) error` — transactional cleanup callback for file removal
- ReindexCallback: exported type `func(txn *lmdb.Txn, orphanedChunkIDs, newChunkIDs []uint64) error` — transactional cleanup callback for reindex
...
- AddFile(path, strategy, idxOpts): check for existing N records via FinalKey — return ErrAlreadyIndexed if present. Allocate fileid, create N/F records. Call Chunker.Chunks (yields Range+Content+Attrs per chunk). For each chunk: fire ChunkCallback with the original Content if present (R473); compute the dedup hash over the original Content alone (R656); when the strategy has a transform, apply it to a copy so its stripped Content feeds trigram/token extraction and its derived Attrs are recorded, while the C record stores the original Content (R646, R647); check H record for dedup — if hit, add fileid to existing C record; if new, allocate chunkid, create H/C records (with derived attrs). Identical original Content dedups; differing tags give distinct original Content and distinct chunkids (R657). Batch T/W updates across all chunks. Update F record with chunk entries and token bag. Returns (fileid, error)
...
- ReadCRecord(txn, chunkID): public power-user accessor — fetch C record by chunkID inside caller's txn, attach db/txn to result for Txn()/DB()/FileRecord navigation (R571)
...
- AddChunker(name, c, transform): register a chunker (Chunker, FileChunker, or both) as a strategy; validates at least one interface is satisfied. Stores the optional transform (nil for none) in the transforms map. In-memory only, I record stores name with empty value. R643, R644
- AddStrategyFunc(name, fn): convenience — wraps fn in FuncChunker, calls AddChunker with a nil transform. R645
- transformFor(strategy): return the registered ContentTransform for a strategy, or nil (also nil for external-command strategies). R646
- indexChunk(c, xf): build a collectedChunk — hash and stored Content are the original chunker bytes; the transform (if any) runs on a copy so its stripped Content feeds trigram/token extraction and its derived Attrs are recorded. Shared by collectChunks, AppendChunks, and the overlay's collectChunksFromContent. R646, R647, R656, R657, R659
- chunkAttrs(chunks, lo, hi): read the stored Attrs for chunks[lo..hi] from their C records in one View txn, keyed by chunkid — retrieval surfaces these (the index-time transform output) without re-running the transform. R655
- transform application (cross-cutting): the transform runs ONLY while indexing. Index paths (AddFile/Reindex via collectChunks, AppendChunks) apply transformFor(strategy) to a copy of each chunk so its stripped Content feeds trigram/token extraction and its derived Attrs are recorded, while the original Content is hashed and stored (R646, R647). The WithIndexedChunkCallback delivery carries the original Content plus the derived Attrs (R659). Retrieval paths (GetChunks streaming + RandomAccessChunker fast path, plus the tmp:// branch getChunksTmp which re-chunks stored raw bytes — R649) do NOT apply the transform: Content is re-read from the file (original), and Attrs are read from the stored C record via chunkAttrs on every path (overlay.chunkAttrs for tmp://), so retrieval surfaces the index-time transform output without re-running it (R655)
- AppendChunks(fileid, content, strategy, opts): chunk content using strategy, for each new chunk: fire ChunkCallback if WithAppendChunkCallback present (R482), hash, check H for dedup, create/update C records. Batch T/W updates. Update F record: append chunk entries, merge token bag, update metadata. Single LMDB write transaction. WithBaseLine adjusts line-based ranges after chunking. AppendAwareChunker dispatch: if the strategy's chunker implements AppendAwareChunker, read the F record's last entry's locator, call chunker.AppendChunks(path, lastLocator, newBytes, yield). When replacedLast=true, drop the F record's last chunk entry and run drop-occurrence cleanup on the dropped chunkid (decrement the chunkid's fileid count in its C record; cascade orphan cleanup via shared helper if that was the last reference). Chunkers that don't implement the interface get the default chunk-newBytes-and-append behavior. The dedup hash is over the original Content alone (R656); when the strategy has a transform, apply it to a copy so its stripped Content feeds trigram/token extraction and its derived Attrs are recorded, while the original Content is hashed and stored (R646, R647). Because a tag line lives in the original Content, a tag edit changes that content, so a recomputed tail hashes differently, gets a fresh chunkid, and fires WithIndexedChunkCallback on the new-chunk path — natural re-index with no Attrs-in-hash workaround (R658).
