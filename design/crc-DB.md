...
- AddFile(path, strategy, idxOpts): check for existing N records via FinalKey — return ErrAlreadyIndexed if present. Allocate fileid, create N/F records. Call Chunker.Chunks (yields Range+Content+Attrs per chunk). When the strategy has a transform, apply it to each yielded chunk first (R646, R647) so the index sees stripped Content. For each chunk: fire ChunkCallback if present (R473), compute the dedup hash over Content + marshaled Attrs (R652), check H record for dedup — if hit, add fileid to existing C record; if new, allocate chunkid, create H/C records (with attrs). Identical Content with differing Attrs yields distinct chunkids (R653). Batch T/W updates across all chunks. Update F record with chunk entries and token bag. Returns (fileid, error)
...
- transform application (cross-cutting): wherever a chunk is produced from raw bytes, apply transformFor(strategy) before indexing or returning it. Index paths (AddFile/Reindex via collectChunks, AppendChunks) apply before hash/trigram/token (R647). Retrieval paths (GetChunks streaming + RandomAccessChunker fast path, plus the tmp:// branch getChunksTmp which re-chunks stored raw bytes — R649) apply after the chunk is produced from the re-read region, so retrieved Content == indexed Content (R648); on the fast path for a transform-carrying strategy the chunk starts with empty Attrs and the transform repopulates them rather than pre-filling stored C-record Attrs (R651)
- AppendChunks(fileid, content, strategy, opts): chunk content using strategy, for each new chunk: fire ChunkCallback if WithAppendChunkCallback present (R482), hash, check H for dedup, create/update C records. Batch T/W updates. Update F record: append chunk entries, merge token bag, update metadata. Single LMDB write transaction. WithBaseLine adjusts line-based ranges after chunking. AppendAwareChunker dispatch: if the strategy's chunker implements AppendAwareChunker, read the F record's last entry's locator, call chunker.AppendChunks(path, lastLocator, newBytes, yield). When replacedLast=true, drop the F record's last chunk entry and run drop-occurrence cleanup on the dropped chunkid (decrement the chunkid's fileid count in its C record; cascade orphan cleanup via shared helper if that was the last reference). Chunkers that don't implement the interface get the default chunk-newBytes-and-append behavior. When the strategy has a transform, apply it to each yielded chunk before hashing (R646, R647); the dedup hash includes Attrs (R652), so a recomputed tail whose Content is unchanged but Attrs changed gets a fresh chunkid and fires WithIndexedChunkCallback on the new-chunk path (R654).
