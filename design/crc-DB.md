...
- settings: loaded from I record (chunking strategies, searchCutoff, caseInsensitive, aliases)
...
- AddFile(path, strategy): chunk file (func or exec), compute trigram counts per chunk, store F/N records, update C counts, write forward + reverse index entries. Returns (fileid, error)
- AddFileWithContent(path, strategy): like AddFile but also returns the file content already read. Returns (fileid, []byte, error)
...
- Search(query, opts): extract trigrams, filter to active set, intersect posting lists, score using ScoreFunc. If WithVerify: tokenize query into terms (space-split, quoted phrases as single terms), read chunk text from disk, discard chunks where any term is absent (case-insensitive substring check). Return SearchResults with IndexStatus
- SearchRegex(pattern, opts): extract trigram query from regex AST (rsc approach), evaluate boolean query against full index, score results, then always verify — read chunk text from disk, run compiled regex, discard non-matches. Return SearchResults with IndexStatus
