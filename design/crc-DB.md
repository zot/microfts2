...
- stock score functions: ScoreOverlap (pure), ScoreBM25 (closure factory), ScoreBigramOverlap (closure factory)
- bigramsEnabled: bool — loaded from I record, gates B record writes and C record bigram section
- BRecord: exported struct {Bigram uint16, ChunkIDs []uint64} — same marshal pattern as TRecord
- BigramEntry: exported struct {Bigram uint16, Count int}
- SearchStrategy: exported struct {Score ScoreFunc, UseBigrams bool} — wraps a ScoreFunc with data requirements
...
## Does
- Create(path, opts): create new database, validate aliases (ASCII-only), set MaxDBs from opts (default 2), write I records with settings using data-in-key pattern. Write bigrams I record (default enabled; --no-bigrams disables). Write version "3" I record
- Open(path, opts): open existing database, set MaxDBs from opts (default 2), load settings from I records including bigramsEnabled flag
...
- AddFile(path, strategy): check for existing N records via FinalKey — return ErrAlreadyIndexed if present. Allocate fileid, create N/F records. Call Chunker.Chunks (yields Range+Content+Attrs per chunk). For each chunk: hash content, check H record for dedup — if hit, add fileid to existing C record; if new, allocate chunkid, create H/C records (with attrs), if bigramsEnabled: extract bigram counts via BigramCounts, store in CRecord.Bigrams, accumulate for B batch. Batch T/W/B updates across all chunks. Update F record with chunk entries and token bag. Returns (fileid, error)
...
- RemoveFile(path): read F record to get chunk list, for each chunkid: read C record, remove fileid — if no fileids remain, delete C/H records and remove chunkid from T/W/B records. Delete F and N records
...
- SearchMulti(query, strategies map[string]SearchStrategy, k, opts): single View txn — collect candidates once (trigram intersection, T/C record reads, chunk filters); if any strategy has UseBigrams, populate bigram counts for candidates. Score with each strategy's ScoreFunc (passing bigram counts when UseBigrams is true), keep top-k per strategy, apply post-filters per result set. Returns []MultiSearchResult
- StrategyFunc(fn ScoreFunc) SearchStrategy: wraps a plain ScoreFunc with UseBigrams=false
- StrategyBigramOverlap(queryBigrams map[uint16]int) SearchStrategy: returns SearchStrategy{Score: ScoreBigramOverlap(queryBigrams), UseBigrams: true}
...
- ScoreBigramOverlap(query): factory — extract query bigrams, return closure that scores CRecord by bigram overlap ratio (matching / total query bigrams)
- WithBigramOverlap(): SearchOption sugar — sets scoring to ScoreBigramOverlap
