...
## Does
- Create(path, opts): create new database via bbolt.Open, validate aliases (ASCII-only), create the fts bucket (CreateBucketIfNotExists), write I records with settings using data-in-key pattern. Write version "2" I record. opts no longer carry MaxDBs/MapSize (R666)
- Open(path, opts): open existing database via bbolt.Open, load settings from I records. opts no longer carry MaxDBs/MapSize (R666)
