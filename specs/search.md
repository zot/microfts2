...
- `FilterByRatio(maxRatio float64, minCount int)`: skips a trigram only when it is **both** non-discriminating **and** expensive to scan — appearing in more than `maxRatio` of total chunks *and* having at least `minCount` postings. The two clauses measure different things on purpose. Discrimination is inherently relative, so the ratio is the right instrument for it; scan cost is absolute, and a fraction tracks it only while the corpus is large enough that a fraction of it is itself a large number. Without the cost clause the rule discards a 6-posting trigram on a 10-chunk corpus while keeping a 97,000-posting one on a large corpus — verdicts four orders of magnitude apart, in the wrong order.

  `minCount` of 0 or 1 disables the cost clause and reproduces pure ratio filtering, since a trigram past the threshold already has at least one posting. The cost clause changes a verdict only where `int(totalChunks × maxRatio) < count < minCount`, which is non-empty only for `totalChunks < minCount / maxRatio`: at `FilterByRatio(0.50, 32)` it is inert above 64 chunks, while at `FilterByRatio(0.01, 32)` it reaches to 3,200. The reach scales with `1/maxRatio`, so a caller filtering at a low ratio should expect the cost clause to be live on a much larger corpus than one filtering at half. Callers choose `minCount` from the scan cost they are willing to pay unconditionally; microfts2 does not pick it for them, because the corpus distribution belongs to the caller.

  Below two chunks ratio filtering cannot discriminate — every trigram present is in 100% of chunks — so the filter returns all trigrams unmodified rather than skipping them all, which would make every query unanswerable on a one-chunk index. This guard is deliberately *independent* of `minCount` rather than folded into it: it rests on the ratio carrying no information, not on the scan being cheap. Collapsing the two would make one-chunk correctness depend on a caller-supplied cost parameter, so a caller passing `minCount` of 0 would silently reopen the defect.

  **What trigram filtering can and cannot cost.** Query trigrams are *intersected*, both within a term and across terms, so keeping a trigram only ever narrows the candidate set. Keeping a high-frequency trigram therefore cannot admit a false positive — it costs the scan of a long posting list and nothing else. Dropping one widens the candidate set, which costs the scoring and post-filtering of extra candidates, but callers running `WithVerify` (or their own string check) see those removed rather than returned. Trigram selection is thus a **throughput** mechanism, not a correctness one, with exactly one exception: if the filter drops *every* query trigram the search collects no candidates at all and returns nothing. No post-filter can repair that, because post-filters only remove. `minCount` bounds that exception only below its reach — above `totalChunks >= minCount / maxRatio` a query whose trigrams are all above the ratio still returns nothing, by design. The alternative, re-running unfiltered when the filter empties a query, is deliberately not provided: it scans precisely the longest posting lists the filter exists to avoid.
